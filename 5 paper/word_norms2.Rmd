...
Previous work on semantic feature production norms in English includes @Buchanan2013, @McRae2005, and @Vinson2008. Norms are created by soliciting participants to list properties or features of a target concept. These features are then compiled into feature sets that are thought to represent, at least somewhat, the memory representation of a particular concept. For example, when queried on what defines a *cat*, participants may list *tail*, *animal*, and *pet*. These features capture the most common types of descriptons: "is a" and "has a". Additionally feature descriptions may include uses, locations, behavior, and gender (i.e., *actor* denotes both a person and gender). The goal of these norms is often to create a set of high-probability features, as there can and will be many idiosyncratic features listed in this task, corresponding to the theory of fuzzy logic for category representation [@Medin1989]. These norms have now been published in Italian [@Montefinese2013], German [and Italian, @Kremer2011], for the blind [@Lenci2013], Spanish [@Vivas2017] and Dutch [@Ruts2004]. 

The data from these studies has been used to explain many semantic based phenomena

<----REWORD THIS however, these data do appear to be particularly
